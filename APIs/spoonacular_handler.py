...
class Spoonacular:
    ...
    def find_by_id(self, id):
        ...
        return {ing['name']: Ingredient(ing['name'], -1, ing['image'], ing['amount']['us']['unit'],
                                        ing['amount']['us']['value'], ing['name'], self.ureg) for ing in
...
class Meal:
    def __init__(self, title, id, image, missedIngredients, usedIngredients, ureg, prebuilt=False):
        self.ureg = ureg
        self.title = title
        self.sp_id = id
        self.image = image
...
class Ingredient:
    ...
    def __sub__(self, right):
        if type(right) in (wallAPI.ProductDetail, Ingredient):
            ...
            return Ingredient(self.name, self.sp_id, self.image, str(amount / amount), amount, self.full_title,
                              self.ureg)
        else:
...
if __name__ == '__main__':
    ...
    # Spoonacular(reg).find_by_id(1105658)
    Spoonacular(reg).search_recipes('pizza')
    pass
